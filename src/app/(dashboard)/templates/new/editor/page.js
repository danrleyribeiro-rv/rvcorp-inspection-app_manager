...
export default function NewTemplateEditorPage() {
  ...
  // Funções para detalhes diretos do tópico
  ...
  const removeDirectDetail = (topicIndex, detailIndex) => {
    setTemplate(prev => ({
      ...prev,
      topics: prev.topics.map((topic, index) =>
        index === topicIndex
          ? {
              ...topic,
              details: topic.details.filter((_, dIndex) => dIndex !== detailIndex)
            }
          : topic
      )
    }));
  ...
  const updateDirectDetailField = (topicIndex, detailIndex, field, value) => {
    setTemplate(prev => ({
      ...prev,
      topics: prev.topics.map((topic, tIndex) =>
        tIndex === topicIndex
  ...
  const updateDirectDetailOptions = (topicIndex, detailIndex, optionsString) => {
    const options = optionsString
      .split(",")
      .map(option => option.trim())
      .filter(Boolean);
    
    updateDirectDetailField(topicIndex, detailIndex, 'options', options);
    updateDirectDetailField(topicIndex, detailIndex, 'optionsText', optionsString);
  };

  // Função para atualizar opções de avaliação do item
  const updateItemEvaluationOptions = (topicIndex, itemIndex, optionsString) => {
    const options = optionsString
      .split(",")
      .map(option => option.trim())
      .filter(Boolean);
    
    updateItemField(topicIndex, itemIndex, 'evaluation_options', options);
    updateItemField(topicIndex, itemIndex, 'evaluation_options_text', optionsString);
  };
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden p-4">
        ...
        <TabsContent value="structure" className="flex-1 overflow-hidden">
          <div className="grid gap-4 h-full" style={{gridTemplateColumns: currentTopic?.direct_details ? '1fr 2fr' : '1fr 1fr 2fr'}}>
            {/* Coluna 1: Tópicos */}
            <div className="border rounded-lg flex flex-col">
              ...
              <ScrollArea className="flex-1 p-2 space-y-1">
                {template.topics?.map((topic, topicIndex) => (
                  <div
                    key={generateStableId('topic', topicIndex)}
            ...
            {/* Coluna 2: Itens (só aparece quando não é direct_details) */}
            {currentTopic && !currentTopic.direct_details && (
              <div className="border rounded-lg flex flex-col">
                <div className="p-3 border-b flex justify-between items-center shrink-0">
                  <div className="flex items-center gap-1 truncate">
                    <span className="text-xs text-muted-foreground hidden lg:inline">Tópico {activeTopicIndex + 1}</span>
                    <ChevronRight className="h-3 w-3 hidden lg:inline" />
                    <span className="font-medium">Itens ({currentTopic?.items?.length || 0})</span>
                  </div>
                  <Button size="sm" variant="outline" onClick={() => addItem(activeTopicIndex)}><Plus className="h-3 w-3" /></Button>
                </div>
                  ...
                  <div className="p-2 space-y-3">
                    <div className="space-y-2 p-2 bg-muted/30 rounded border">
                      ...
                      <div className="flex items-center justify-between">
                        <Label className="text-xs">Tópico com detalhes diretos</Label>
                        <Switch
                          checked={currentTopic.direct_details || false}
                          onCheckedChange={checked => updateTopicField(activeTopicIndex, 'direct_details', checked)}
                          className="h-4 w-8"
                        />
                      </div>
                    </div>
                    <div className="space-y-1">
                      {currentTopic.items?.map((item, itemIndex) => (
                        <div
                          key={generateStableId('item', itemIndex)}
                          className={`p-2 rounded cursor-pointer border ${
            ...
            <div className="border rounded-lg flex flex-col">
              <div className="p-3 border-b flex justify-between items-center shrink-0">
                  ...
                  {currentTopic?.direct_details ? (
                    ...
                    currentItem && (
                      <>
                        <span className="text-xs text-muted-foreground hidden lg:inline">Tópico {activeTopicIndex + 1}</span>
                        <ChevronRight className="h-3 w-3 hidden lg:inline" />
                        <span className="text-xs text-muted-foreground hidden lg:inline">Item {activeItemIndex + 1}</span>
                        <ChevronRight className="h-3 w-3 hidden lg:inline" />
                ...
                {((currentTopic?.direct_details && currentTopic) || (!currentTopic?.direct_details && currentItem)) && (
                  <Button size="sm" variant="outline" onClick={() => {
                    if (currentTopic?.direct_details) {
                      addDirectDetail(activeTopicIndex);
                    } else {
                      addDetail(activeTopicIndex, activeItemIndex);
                    }
                  }}>
                ...
                {currentTopic?.direct_details ? (
                  ...
                  <div className="p-2 space-y-3">
                    ...
                    <div className="space-y-3">
                      ...
                      {currentTopic.details?.map((detail, detailIndex) => (
                        <div key={generateStableId('detail', detailIndex)} className="border rounded-md p-3 space-y-3 bg-card shadow-sm">
                          <div className="flex justify-between items-center">
                            ...
                            <Button size="icon" variant="ghost" className="h-6 w-6 p-0 text-destructive hover:text-destructive/80" onClick={() => removeDirectDetail(activeTopicIndex, detailIndex)}>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                              ...
                              <Input 
                                value={detail.name} 
                                onChange={e => updateDirectDetailField(activeTopicIndex, detailIndex, 'name', e.target.value)} 
                                className="h-8 text-sm mt-1" 
                              />
                            </div>
                            <div>
                              <Label className="text-xs">Tipo de Resposta</Label>
                              <Select value={detail.type} onValueChange={value => updateDirectDetailField(activeTopicIndex, detailIndex, 'type', value)}>
                          ...
                          {detail.type === "select" && (
                            <div>
                              ...
                              <Input 
                                ...
                                onChange={e => updateDirectDetailOptions(activeTopicIndex, detailIndex, e.target.value)} 
                      ...
                      {currentTopic.details?.length === 0 && <p className="text-sm text-muted-foreground p-4 text-center">Nenhum detalhe adicionado a este tópico.</p>}
                    </div>
                  </div>
                ) : (
                  // Detalhes do item
                    ...
                    <div className="p-3 space-y-4">
                      <div className="space-y-3 p-3 bg-muted/30 rounded border">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Switch
                            checked={currentItem.evaluable || false}
                            onCheckedChange={checked => updateItemField(activeTopicIndex, activeItemIndex, 'evaluable', checked)}
                            className="h-4 w-8"
                          />
                        </div>
                        {currentItem.evaluable && (
                          <div>
                            <Label className="text-xs">Opções de avaliação (separadas por vírgula)</Label>
                            <Input
                              value={currentItem.evaluation_options_text || currentItem.evaluation_options?.join(", ") || ""}
                              onChange={e => updateItemEvaluationOptions(activeTopicIndex, activeItemIndex, e.target.value)}
